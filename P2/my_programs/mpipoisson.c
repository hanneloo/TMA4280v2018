/*                      TMA4280 Introduction to supercomputing
  ...
  This program is a C program to solve the two-dimenstional Poisson equation on a 
   ...
   This particular version uses MPI-implementation in order to solve the problem
                                 with P processes.
                                                                                */
...
int main(int argc, char **argv)
{
  ...
  // Characteristics of the MPI procedure
  ...
  if (!is_pow2(n))
    {
      if (rank == 0)
	{
	  printf("The problemsize is not the power of 2. Aborting \n");
	}
